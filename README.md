...
- **Graph-level Metrics**
  ...
  Let a **test set** be the set of models generated by an  approach. Given a test set $I$, which contains $n_I$ graphs, the graph-level metrics are defined as follows. 
  
  - **Number of Operations (NOO)**, **Number of Operation Types (NOT)**, **Number of Operation Pairs (NOP)**, **Number of Shapes and Parameters (NSP)**. We put these metrics together because they all represent the diversity of a computation graph. The NOO of a graph $g$ is defined as the total number of operations in $g$. The NOT of a graph $g$ is defined as the number of different types of operations in $g$. The NOP of a graph $g$ is defined as the number of different edges in $g$ that connect two operations together. The NSP of a graph $g$ is defined as the total number of different shapes of input tensors and parameters for each operation in graph $g$. These metrics for test set $I$ are defined as the average of each of them of all the graphs in $I$: $NOX(I) = \frac{\Sigma_g{NOX_g(g)}}{n_I}$, where $NOX \in \{NOO, NOT, NOP, NSP\}$.
  - **Number of Tensor Paths (NTP)**. Given a graph $g$ and an input tensor $t$ of the model, we call a route that $t$ goes along from the input to the output of the model a **Path** of tensor $t$ in $g$. Note that there may be more than one route from an input tensor $t$ to the output of the model  and we take them as different paths. The NTP of a graph $g$ is defined as the number of different paths in $g$. The NTP metric shows the number of different manipulations of an input tensor, which indicates the complexity of a model. The NTP of $I$ is defined as the average of NTP of all the graphs in $I$: $NTP(I) = \frac{\Sigma_g{NTP_g(g)}}{n_I}$.
  - **Max Length of Paths (MLP)**. Given a path $p$ in a graph $g$, the length of $p$ is defined as the number of operations in this path. The MLP metric and the ALP metric below present the depth of the model. The MLP of graph $g$ is defined as the length of the longest path in $g$. The MLP of $I$ is defined as the average of MLP of all the graphs in $I$: $MLP(I) = \frac{\Sigma_g{MLP_g(g)}}{n_I}$.
  - **Average Length of Paths (ALP)** of graph $g$ is defined as the average of the lengths of all the paths in $g$: $ALP_g(g) = \frac{\Sigma_p{len(p)}}{n_p}$, where $p$ is a path in $g$ and $n_p$ is the number of different paths in $g$. The ALP of $I$ is defined as the average of ALP of all the graphs in $I$: $ALP(I) = \frac{\Sigma_g{ALP_g(g)}}{n_I}$.
  
- **Operation-level Metrics**
  
  An **operation corpus** is a set of operations with their attributes including the operation name and the possible number of different input degrees. Given an operation corpus $C$ containing $n_C$ different operations and a test set $I$, 
  we first give the definition of each metric on an operator $o$ in $I$, denoted as $XXC_{op}(o)$, and the operation-level metric of $I$ is defined as the average of the metric on different operators, i.e.,  $XXC(I) = \frac{\Sigma_o{XXC_{op}(o)}}{n_C}$, where $XXC \in \{OTC,IDC,ODC,SEC,SPC\}$.
  
  - **Operation Type Coverage (OTC)** shows the diversity of operations in the test set $I$. Let ${OTC}_{op}(o)$ be 1 if operation $o$ appears in $I$, and 0 otherwise.
  - **Input Degree Coverage (IDC)** shows the diversity of the input degrees of the operations in the test set. For each operation $o$ in $I$, let $n_{id}(o, I)$ and $n_{id}(o, C)$ be the number of different input degrees of $o$ in $I$ and in $C$ respectively, we have the IDC of the operation $o$ as the ratio of $n_{id}(o, I)$ and $n_{id}(o, C)$: ${IDC}_{op}(o) = \frac{n_{id}(o, I)}{n_{id}(o, C)}$. 
    For operations that have infinite numbers of input degrees  (e.g. `Max`), we set an upper bound for their $n_{id}(o, C)$.
  - **Output Degree Coverage (ODC)** shows the diversity of output degrees of operations in $I$.  The ${ODC}_{op}(o)$ is defined as the number of different output degrees of $o$ in $I$. 
  - **Single Edge Coverage (SEC)** shows the diversity of edges between the operations in $I$. Let $n_e(o, I)$ be the number of different edges directed from the operation $o$ in $I$. Supposing that each operation could be connected with all operations in $C$,  we define the SEC of $o$ as the ratio of $n_e(o, I)$ and $n_C$: ${SEC}_{op}(o) = \frac{n_e(o, I)}{n_C}$. 
  - **Shapes and Parameters Coverage (SPC)** indicates the diversity of input and output tensor shapes of the operations in the test set. The ${SPC}_{op}(o)$ is defined as the number of different input tensor shapes and parameters of  $o$ in $I$. 
